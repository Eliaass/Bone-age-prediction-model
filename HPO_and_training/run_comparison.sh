...
#SBATCH --job-name=test_model_comparison
...
SCRIPT_PATH="$PROJECT_DIR/model_comparison_testset.py"
...
if [ ! -f "$SCRIPT_PATH" ]; then
    echo "ERROR: Test comparison script not found:"
    echo "$SCRIPT_PATH"
    exit 1
fi

if [ ! -f "$PROJECT_DIR/boneage-test-dataset.csv" ]; then
    echo "ERROR: Test CSV file not found:"
    echo "$PROJECT_DIR/boneage-test-dataset.csv"
    exit 1
fi

if [ ! -d "$PROJECT_DIR/cropped_overlayed_RSNA_testset_512x512" ]; then
    echo "ERROR: Test image directory not found:"
    echo "$PROJECT_DIR/cropped_overlayed_RSNA_testset_512x512"
    exit 1
fi
...
echo "=== Starting ConvNeXtV2-Tiny final test-set comparison ==="

python -u "$SCRIPT_PATH"

echo "=== ConvNeXtV2-Tiny final test-set comparison finished ==="
echo "Date: $(date)"