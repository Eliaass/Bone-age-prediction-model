...
#SBATCH --job-name=test_convnextv2_tiny
...
SCRIPT_PATH="$PROJECT_DIR/convnextv2_tiny_test_with_calibration.py"
...
if [ ! -f "$PROJECT_DIR/boneage-training-dataset.csv" ]; then
    echo "ERROR: CSV file not found:"
    echo "$PROJECT_DIR/boneage-training-dataset.csv"
    exit 1
fi

if [ ! -d "$PROJECT_DIR/cropped_overlayed_RSNA_dataset_1024x1024" ]; then
    echo "ERROR: Image directory not found:"
    echo "$PROJECT_DIR/cropped_overlayed_RSNA_dataset_1024x1024"
    exit 1
fi
...
echo "=== Starting ConvNeXtV2-Tiny test inference with linear calibration ==="

python -u "$SCRIPT_PATH"

echo "=== ConvNeXtV2-Tiny test inference finished ==="
echo "Date: $(date)"